...
`execute-batch!` does not check that each row of values in the parameter groups has the same number of values as the prepared statement expects. If your first row has too few values, you'll get an exception when the prepared statement is executed. If subsequent rows have too few values, the previously supplied values will be used for the missing parameters, because JDBC reuses parameter values across the batch, if they are not overridden for every column. If you want that validation on an insert, look at `insert-multi!` in the `next.jdbc.sql` namespace, which does check that all rows have the same number of values and will throw an exception if they do not (and can be told to use `execute-batch!` under the hood).
