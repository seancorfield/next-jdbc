...
* `:pre-execute-fn` -- a function that can pre-process the SQL & parameters vector and the options hash map before the SQL is executed; the default is to return both unchanged in a vector pair,
* `:post-execute-fn` -- a function that can post-process the `ResultSet` object and the options hash map after the SQL is executed; the default is to return both unchanged in a vector pair,
